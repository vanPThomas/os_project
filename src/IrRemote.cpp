...
{
    HardwareUtil::init_input_pin_with_pullup(_pin);
}
...
uint8_t IrRemote::readRawButtonId()
{
    ...
    while (HardwareUtil::read_gpio(_pin) && (HardwareUtil::get_current_us() - now < _timeout_ms * 1000ULL)) {}
    if (HardwareUtil::read_gpio(_pin)) return 0;

    // Header low (~9 ms)
    start = HardwareUtil::get_current_us();
    while (!HardwareUtil::read_gpio(_pin)) {}
